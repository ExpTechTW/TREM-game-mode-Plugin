class Plugin {
  ...
  checkPipMode() {
    ...
    if (!button) return;
    const pipMode = localStorage.getItem("pip-mode");
    const TREM = this.#ctx.TREM;
    if (pipMode === "true") {
  ...
  addClickEvent(TREM) {
    ...
    button.addEventListener("click", () => {
      if (TREM.constant.GAME_MODE) {
        TREM.constant.GAME_MODE = false;
        button.title = "啟用 pip 功能";
        localStorage.setItem("pip-mode", "false");
      } else {
        TREM.constant.GAME_MODE = true;
        button.title = "停用 pip 功能";
        localStorage.setItem("pip-mode", "true");
      }
    });
  }
}
